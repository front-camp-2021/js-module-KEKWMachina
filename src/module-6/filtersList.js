export default class FiltersList {
  ...
  get template() {
    return `<div class="checkbox__wrapper"><div data-element="body">
    </div></div>`;
  }
  ...
  update(list = []) {
    ...
    const listItems = list.map(item => {
      ...
      if(item.checked) {
        listElement.innerHTML = `<input type="checkbox" id="${item}" name="$10" class="filters__checkbox-square" checked>
      <label for="${item}">${item}</label>`;
      } else {
        listElement.innerHTML = `<input type="checkbox" id="${item}" name="$10" class="filters__checkbox-square">
      <label for="${item}">${item}</label>`;
      }
